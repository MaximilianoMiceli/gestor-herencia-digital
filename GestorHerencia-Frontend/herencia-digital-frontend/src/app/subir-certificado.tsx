  ...
  useEffect(() => {
    ...
    const cargarTitulares = async () => {
      try {
        ...
        // aceptada (misma regla de negocio que aplica el backend): se deduplica por
        // titularId porque un mismo titular puede haberme asignado varios activos.
        const aceptadas = herencias.filter((h) => h.estado === 'Aceptado');
        const mapa = new Map<number, TitularElegible>();
        aceptadas.forEach((h) => mapa.set(h.titularId, { titularId: h.titularId, titularNombre: h.titularNombre }));
          ...
          {titularesElegibles.length === 0 ? (
            <View style={styles.emptyCard}>
              <Text style={styles.emptyText}>
                Todavía no aceptaste ninguna herencia. Primero aceptá una invitación desde
                "Mis herencias" para poder subir un certificado.
              </Text>
            </View>
          ) : (
            <>
